...
from ..models import db, Customer, Contract, Invoice, Transaction, invoices_transactions
...
def show(id: int):
    ...
    contract_results = [contract.serialize() for contract in contracts]
    invoice_results = [invoice.serialize() for invoice in invoices]
    if len(contract_results) == 0:
        return render_template('show_customer.html', customer=customer, contracts=None, invoices=None, transactions=None)
    if len(invoice_results) == 0:
        return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=None, transactions=None)
    return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=invoices, transactions=None)
...
def customer_contracts(id: int):
    customer = Customer.query.get_or_404(id)
    contracts = Contract.query.filter_by(customer_id=customer.id)
    invoices = Contract.query.filter_by(customer_id=customer.id)
    result = [contract.serialize() for contract in contracts]
    if len(result) == 0:
        return render_template('show_customer.html', customer=customer, contracts=None, invoices=None)
    return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=invoices)
...
def customer_invoices(id: int):
    ...
    if len(result) == 0:
        return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=None)
    return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=invoices)


@bp.route('/<int:customer_id>/contracts/<int:contract_id>/invoices', methods=['POST'])
def contract_invoices(customer_id: int, contract_id: int):
    customer = Customer.query.get_or_404(customer_id)
    contracts = Contract.query.filter_by(customer_id=customer.id)
    invoices = Invoice.query.filter_by(contract_id=contract_id)
    result = [invoice.serialize() for invoice in invoices]
    if len(result) == 0:
        return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=None)
    return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=invoices)
...
@bp.route('/<int:customer_id>/contracts/<int:contract_id>/invoices/<int:invoice_id>/transactions', methods=['POST'])
def invoice_transactions(customer_id: int, contract_id: int, invoice_id: int):
    customer = Customer.query.get_or_404(customer_id)
    contracts = Contract.query.filter_by(customer_id=customer.id)
    invoices = Invoice.query.filter_by(contract_id=contract_id)
    ...
    if len(result) == 0:
        return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=None, transactions=None)
    return render_template('show_customer.html', customer=customer, contracts=contracts, invoices=invoices, transactions=transactions)
